...
class CopyTaskData:
    def generate_batches(self, num_batches, batch_size, bits_per_vector=8, curriculum_point=20, max_seq_len=20,
        ...
        for i in range(num_batches):
            ...
            labels = []
            for input in inputs:
                labels.append(np.asarray(self.change2Preorder(1,input)))
            ...
            batches.append((pad_to_len, full_inputs, labels[:, :, :bits_per_vector]))
        return batches
    ...
    def change2Preorder(self,node_index,target_list):
        list_length = len(target_list)
        res = []
        res.append(target_list[node_index-1])
        if node_index*2 > list_length:
            return res
        left_child = self.change2Preorder(node_index*2,target_list)
        res = res+left_child
        if node_index*2+1 > list_length:
            return res
        right_child = self.change2Preorder(node_index*2+1,target_list)
        res = res+right_child
        return res
    ...
    def generate_batches(self, num_batches, batch_size, bits_per_vector=6, curriculum_point=6, max_seq_len=6,
        ...
        for i in range(num_batches):
            ...
            def generate_item(seq_len):
                ...
                output_item = items[int((query_item_num-1)/2)]

                inputs = [sub_item for item in items for sub_item in item]

                return inputs, query_item, list(map(lambda sub_item: sub_item[:bits_per_vector], output_item))
...
if __name__ == '__main__':
    ...
    labels = data_generator.change2Preorder(1,list)
    print(labels)'''
